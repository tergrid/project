...
const PapaSamay = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const url = 'https://api.vedicastroapi.com/v3-json/dosha/papasamaya';
    const params = {
      lang: 'en',
    ...
    fetch(`${url}?${new URLSearchParams(params)}`)
      ...
      .then((data) => {
        if (data.status === 200 && data.response) {
          ...
          setResult({ error: 'No Papasamaya data available.' });
        }
      })
      .catch((error) => {
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit">Check Papasamaya</button>
      </form>
        ...
        <div id="result" className="result">
          {result.error ? (
            ...
            <div className="result-card">
              <h4>Papasamaya Factors:</h4>
              <ul>
                <li><strong>Rahu Papa:</strong> {result.rahu_papa}</li>
                <li><strong>Sun Papa:</strong> {result.sun_papa}</li>
                <li><strong>Saturn Papa:</strong> {result.saturn_papa}</li>
                <li><strong>Mars Papa:</strong> {result.mars_papa}</li>
              </ul>
            </div>
          )}
        </div>
